...
class AnimationLogoView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        mOffsetDuration =
            ta.getInt(R.styleable.AnimationLogoView_offsetAnimDuration, ANIM_LOGO_DURATION)
        mGradientDuration = ta.getInt(
            R.styleable.AnimationLogoView_gradientAnimDuration,
            ANIM_LOGO_GRADIENT_DURATION
        )
        mTextColor = ta.getColor(R.styleable.AnimationLogoView_textColor, ANIM_LOGO_TEXT_COLOR)
        mGradientColor =
            ta.getColor(R.styleable.AnimationLogoView_gradientColor, ANIM_LOGO_GRADIENT_COLOR)
        mTextPadding = ta.getDimensionPixelSize(
            R.styleable.AnimationLogoView_textPadding,
    ...
    private fun initLogoCoordinate() {
        ...
        for (i in 0 until mLogoTexts.size()) {
            mRandomPoints.put(
                i,
                PointF(Math.random().toFloat() * mWidth, Math.random().toFloat() * mHeight)
            )
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        if (!isOffsetAnimEnd) {
            ...
            for (i in 0 until mQuietPoints.size()) {
                val point = PointFEvaluator().evaluate(
                    mOffsetAnimProgress,
                    mRandomPoints[i],
                    mQuietPoints[i]
                )
