...
/**
 * @param T 目标活动页
 * @param flags 调用startActivityForResult时有用到
 * @param extra 调用Intent.putExtras(@NonNull Bundle extras)时用到
 * @param value 需要putExtra一个数据时候使用，具体查看扩展函数实现 getIntent<T>(flags, extra, list)
 * @param values 需要putExtra许多数据时使用，具体查看扩展函数实现 getIntent<T>(flags, extra, list)
 */
